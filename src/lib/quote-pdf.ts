...
export async function downloadQuotePdf(data: QuotePdfInput) {
  ...
  doc.setFontSize(10);
  y += 7;
  doc.text(`${data.fromName}  →  ${data.toName}`, 22, y);
  y += 5;
  doc.setTextColor(110, 110, 115);
  doc.text(`${data.km} km · delai ${data.delay}`, 22, y);
  if (data.extras) {
    y += 5;
    doc.text(`Options : ${data.extras}`, 22, y);
  }
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(22);
  doc.text(`${data.range.low} – ${data.range.high} EUR`, 32, y + 26);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(110, 110, 115);
  ...
  const disclaimer = doc.splitTextToSize(
    "Ce document n'est pas un devis ferme. La fourchette depend du trafic, des attentes, du vehicule et du retour a vide. Un professionnel confirme le prix avant toute mission. Inclus a la confirmation : conduite, carburant, peages, retour convoyeur, etat des lieux photo, remise des cles. Franchise de TVA art. 293 B du CGI.",
    166,
  );
