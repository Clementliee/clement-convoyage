  ...
  return (
    <div className="rounded-2xl border border-line bg-surface p-5 shadow-sm sm:p-8">
      <div className="mb-6">
        ...
        <h2 className="mt-4 font-display text-2xl text-navy">{STEPS[step]}</h2>
      </div>
      ...
      {quote.ok && (step >= 3 || kmManual) ? (
        <div className="mt-8 rounded-2xl bg-navy p-6 text-surface">
          <p className="text-sm text-surface/70">
            {quote.fromName} → {quote.toName} · {quote.km} km · {quote.delay}
            {client === "part" ? " · particulier" : " · professionnel"}
          </p>
          <p className="mt-2 font-display text-5xl font-semibold tabular-nums text-coral">{formatEuro(quote.total)}</p>
          <p className="mt-2 text-sm text-surface/80">TTC, franchise TVA art. 293 B du CGI. Estimation indicative.</p>
          <p className="mt-4 text-xs text-surface/60">
            Inclus : conduite, carburant, péages, retour, EDL photo, remise des clés.
          </p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <Button asChild>
              <a
                href={`mailto:${SITE.email}?subject=Devis ${quote.fromName} ${quote.toName}&body=Demande de confirmation : ${quote.fromName} vers ${quote.toName}, ${formatEuro(quote.total)}.`}
              >
                Recevoir ce devis par e-mail
              </a>
            </Button>
            <Button variant="ghost" className="border-surface/20 bg-surface/10 text-surface" asChild>
    ...
    <div className="grid gap-3">
      {options.map((o) => (
        <button
          ...
          type="button"
          onClick={() => onPick(o.v)}
          className="flex items-center justify-between rounded-2xl border border-line bg-bg px-4 py-4 text-left hover:border-coral"
        >
  ...
  return (
    <label className="block text-sm text-muted">
      ...
      <input
        ...
        className="mt-1 w-full rounded-xl border border-line bg-bg px-3 py-3 text-base text-navy"
        placeholder={label}
        autoComplete="off"
      />
      <datalist id={`${id}-list`}>
...
function Toggle({ label, on, onClick }: { label: string; on: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex w-full items-center justify-between rounded-2xl border px-4 py-4 text-left ${on ? "border-coral bg-sand" : "border-line bg-bg"}`}
    >
